...
def get_summary(transcript):
    try:
        ...
        print(transcript)
        prompt =  f""" You are a professional summarizer. Your task is to summarize the following transcript in a concise and meaningful way. Focus on capturing the main topics, key points, and significant details, while avoiding unnecessary repetition or minor details. Ensure the summary is coherent and delivers the essence of the conversation. Do not include filler phrases like 'thank you' or any pleasantries. Keep it to 3-5 sentences. Transcript: {transcript} Summary"""
        print(prompt)
...
def get_title(title):
    try:
        ...
        prompt = f"Title and Subcategories Extraction: Based on the provided transcript: {title}, extract a single title that encapsulates the main theme or key focus of the content. In addition, provide 3 to 4 subcategories that highlight the most important topics or sections discussed. The main title should be concise, while each subcategory should be a short phrase reflecting a key subtopic. Ensure that both the title and subcategories are clear, direct, and representative of the key content. Transcript:{title} Expected Output like this: Title: [Main Title] Subcategories: 1. [Subcategory 1] 2. [Subcategory 2] 3. [Subcategory 3] 4. [Subcategory 4]"
...
def upload_to_watson_discovery(transcript, video_filename):
    print("uploading to discovery...............")
    try:
        # Create a .txt file with the transcript
        transcript_filename = f"{video_filename}_transcript.txt"
        with open(transcript_filename, 'w') as file:
            file.write(transcript)
        
        # Upload the .txt file to Watson Discovery
        ...
        print(f"Successfully uploaded to Watson Discovery: {response}")
    except Exception as e:
        print(f"Error uploading to Watson Discovery: {e}")
...
def upload():
    ...
    if file and allowed_file(file.filename):
        ...
        upload_to_watson_discovery(a,filename)
        #----------------------transcript end----------------------
...
def process_video(video_path):
    try:
        ...
        keyfortext = "KhfzBYwhvM7DmKPtUzE6GaUa3SjwN6n22KGQsJ25_axH"
        urlfortext = "https://api.eu-gb.speech-to-text.watson.cloud.ibm.com/instances/111fd89b-d6f0-4398-9008-eb45d9ce65d7"
